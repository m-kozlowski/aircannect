...
constexpr uint32_t REQUIRED_RX_FILTER_CODE =
    ((AC_CAN_RX_ID & AC_CAN_BOOT_ID) & CAN_STANDARD_ID_MASK)
    << CAN_STANDARD_ID_SHIFT;
constexpr uint32_t REQUIRED_RX_FILTER_MASK =
    (((AC_CAN_RX_ID ^ AC_CAN_BOOT_ID) & CAN_STANDARD_ID_MASK)
     << CAN_STANDARD_ID_SHIFT) |
    CAN_FILTER_TRAILING_BITS_MASK;

constexpr bool required_rx_filter_accepts(uint32_t id) {
    const uint32_t encoded =
        (id & CAN_STANDARD_ID_MASK) << CAN_STANDARD_ID_SHIFT;
    return ((encoded ^ REQUIRED_RX_FILTER_CODE) &
            ~REQUIRED_RX_FILTER_MASK) == 0;
}
...
// AirMini binary NCP responses use the standard 0x380 identifier.
...
bool CanDriver::set_debug_log_rx_enabled(bool enabled) {
    ...
    if (err != ESP_OK) {
        Log::logf(CAT_CAN, LOG_WARN, "RX filter status failed: %s\n",
                  esp_err_name_short(err));
        return false;
    }
    ...
    err = twai_stop();
    if (err != ESP_OK) {
        Log::logf(CAT_CAN, LOG_WARN, "RX filter stop failed: %s\n",
                  esp_err_name_short(err));
        return false;
    }
    ...
    if (err != ESP_OK) {
        ...
        if (restore_err != ESP_OK) {
            (void)recover_or_restart("CAN RX filter transition failed");
        }
        return false;
    }
